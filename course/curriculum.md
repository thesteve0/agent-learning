...
## Module 5: Evaluating Your Agent System (1.5-2 hours)

### Learning Objectives

- Apply the compound reliability formula to predict and diagnose system accuracy
- Build a reusable evaluation harness that measures accuracy at step, field, and end-to-end levels
- Establish a baseline MLflow run before any further changes
- Use the harness to objectively assess future improvements (prompts, fine-tuning, model swaps)

### Why Evaluate Before You Integrate?

You are about to wire the orchestrator into a web API. Once that happens, failures become user-facing. You need a baseline now so that regressions are immediately visible and so that fine-tuning results can be proven, not guessed.

The compound reliability formula from production research (Rajan, 2026):

```
overall_accuracy = per_step_accuracy ^ number_of_steps

Example: 0.90 × 0.90 × 0.90 = 72.9% end-to-end
```

Nearly 1 in 4 screenshots returns wrong data at 90% per-step accuracy. See `resources/TheMulti-Agent Trap _ TowardsDataScience.pdf` for the full analysis.

### Hands-On Activity

**Exercise**: `exercises/exercise4_evaluation.md` (you build this)

**Tasks**:
1. Create `tests/fixtures/eval_set.json` with labeled screenshots
2. Implement `evaluate_pipeline()` measuring step, field, and end-to-end accuracy
3. Log results to MLflow as a named baseline run
4. Run the compound reliability check

**Key code patterns** (full walkthrough in `modules/module5-evaluation.md`):

```python
def evaluate_pipeline(orchestrator, eval_set):
    """Measure accuracy at three levels: step, field, end-to-end."""
    results = {"e2e_correct": 0, "field_scores": defaultdict(list), "errors": []}
    for example in eval_set:
        extracted = orchestrator.analyze_screenshot(example["screenshot"])
        for field, expected in example["expected_output"].items():
            results["field_scores"][field].append(extracted.get(field) == expected)
    # compute e2e_accuracy, field_accuracy, error_rate
    return results
...
mlflow ui --port 5000  # verify baseline run appears
```

### Key Takeaways

1. **Measure three levels** - step, field, and end-to-end; each diagnoses different problems
2. **Field accuracy targets improvements** - lowest-scoring field = where to focus fine-tuning
3. **MLflow enables before/after comparison** - one run is useless without a comparison point
4. **Build the test set incrementally** - add examples as you encounter failure modes

### Exercise

**File**: `exercises/exercise4_evaluation.md`

Build the full harness: eval set creation, `evaluate_pipeline()`, MLflow logging, compound reliability check.

---

## Module 6: FastAPI Integration (1-2 hours)
...
## Module 7: Advanced & Alternatives (1-2 hours)
...
**File**: `examples/module7_conference_demo.py`

```python
"""
Module 7: Conference demo progression.
...
```bash
python examples/module7_conference_demo.py
```
